...
def lookupClosure(k,v) :
  """
  """
  if k in heap :
    return heap[k]
  else :
    msg = "Invalid lookup closure %s", k
    crash(msg)

def allocateClosure(ns, kind, il, cl, ld) :
...
def lookup(handle, field) :
    ...
    elif isinstance(heap[handle], list) :
      listlen = len(heap[handle])
      pns = heap[handle][listlen - 1]
    else :
      pns = heap[handle]["parentns"]
    #print "pns: ", pns
    if pns == "nil" :
      msg = "invalid lookup address: " + handle + " " + field
      crash(msg)
    else :
...
def resolveNS(uid) :
  ...
  """
  for key in heap :
    if isinstance(heap[key], dict) :
      if uid in heap[key].keys() :
        return key
...
def store(handle, field, rval) :
    ...
    """stores rval at heap[handle][field], provided that
         ...
         (ii) the type of  rval  matches the type of what's already stored at
              heap[handle][field] (else crashes with a type-error message)
       params:  handle, field, as described above
                rval -- an int or a handle
    """
    if isLValid(handle, field) :
